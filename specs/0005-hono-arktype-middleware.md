# 0005 — Hono ArkType Serialization Middleware
...
`@eidora/hono-arktype-middleware` exports a Hono middleware named `serialize`.
It rewrites the top-level `data` property of a JSON response with Eidora's
ArkType-backed serializer after the route handler completes.
...
`serialize` accepts an ArkType object schema supported by `@eidora/arktype`
and uses Hono's `createMiddleware` factory to return a `MiddlewareHandler`.
...
- ArkType morph exceptions propagate through Hono's normal error handling.

## Required Behavior

- Unknown, missing, and invalid root properties are omitted by the ArkType
  adapter.
- Successful ArkType constraints, morphs, defaults, and nested validation are
  preserved.
